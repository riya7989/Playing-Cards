...
public class GameClass implements Game {
	 ...
	 * @@throws IllegalArgumentException if number of decks is less than 1, number
	 *                                   of hands is negative or number of cards is
	 *                                   negative/more than the amount in deck
	 */
	public GameClass(String deckType, int numberOfDecks, int numberOfHands, int numberOfCards) {
	...
	public void createDeck(String deckType) {
		...
		if (deckType.equalsIgnoreCase("standard")) {
			gameDeck = new StandardDeckClass();
		} else if (deckType.equalsIgnoreCase("euchre")) {
			gameDeck = new EuchreDeckClass();
		} else if (deckType.equalsIgnoreCase("pinochle")) {
			gameDeck = new PinochleDeckClass();
		} else if (deckType.equalsIgnoreCase("vegas")) {
			gameDeck = new VegasDeckClass();
		} else {
			throw new IllegalArgumentException(deckType + " is not a valid deck type.");
		}
	}
	...
	public void createDeck(String deckType, int numberOfDecks) {
		...
		if (numberOfDecks < 2) {
			...
			if (deckType.equalsIgnoreCase("standard")) {
				gameDeck = new StandardDeckClass(numberOfDecks);
			} else if (deckType.equalsIgnoreCase("euchre")) {
				gameDeck = new EuchreDeckClass(numberOfDecks);
			} else if (deckType.equalsIgnoreCase("pinochle")) {
				gameDeck = new PinochleDeckClass(numberOfDecks);
			} else if (deckType.equalsIgnoreCase("vegas")) {
				gameDeck = new VegasDeckClass(numberOfDecks);
			} else {
				throw new IllegalArgumentException(deckType + " is not a valid deck type.");
			}
		}
	 ...
	 */
	protected int getNumberOfHands() {
	 ...
	 */
	protected List<Hand> getGameHand() {
